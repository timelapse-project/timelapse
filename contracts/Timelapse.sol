...
contract Timelapse is Ownable {
     ...
     * @dev Accept the offer `_offerId` (By choosing proposal `_proposalId`) of a customer (identified with `_phoneHash`) with reference `_ref` at timestamp `_acceptanceTimestamp`
     */
    function acceptance(
        address _phoneHash,
        string memory _ref,
    ...
    function getCustomerActivitiesLog(
        ...
        for (uint8 i = 0; i < billing.getHistorySize(_phoneHash); i++) {
            ...
            if (
                paidTimestamp >= _startTimestamp &&
                paidTimestamp <= _endTimestamp
            ) {
                customerActivitiesSize++;
            }
        }
        CustomerActivity[] memory customerActivities = new CustomerActivity[](
            customerActivitiesSize
        );
        for (uint8 i = 0; i < offering.getOfferSize(_phoneHash); i++) {
            uint256 offeringIndex = offering.offerList(_phoneHash, i);
            ...
            if (timestamp >= _startTimestamp && timestamp <= _endTimestamp) {
                ...
                uint256 numberProposal = offering.getProposalOfferSize(
                    offeringIndex
                );
        ...
        for (uint256 i = 0; i < billing.getHistorySize(_phoneHash); i++) {
            ...
            if (
                ...
                customerActivities[customerActivitiesIndex].log = string(
        ...
        returns (Invoice[] memory)
    {
        ...
        for (
            uint256 i = billing.getHistoriesSize();
            i > 0 && !(outOfInvoicingWindow);
            i--
        ) {
            (
                ,
        ...
        returns (Reporting[] memory)
    {
        ...
        for (
            uint256 i = offering.getOffersSize();
        ...
        for (
            uint256 i = billing.getHistoriesSize();
            i > 0 && (!(outOfReportingWindow2) || !(outOfReportingWindow3));
            i--
        ) {
            (
                ,
